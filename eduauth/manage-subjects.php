...
?>
     ...
     <?php include_once('includes/header.php');?>
      ...
      <div class="container-fluid page-body-wrapper">
       ...
       $ret = "SELECT ID FROM tblsubject";
$query1 = $dbh -> prepare($ret);
$query1->execute();
$results1=$query1->fetchAll(PDO::FETCH_OBJ);
$total_rows=$query1->rowCount();
$total_pages = ceil($total_rows / $no_of_records_per_page);
$sql="SELECT tblsubject.SubID,tblsubject.ID as sid,tblsubject.SubjectName,tblsubject.StudentClass from tblsubject join tblclass on tblclass.ID=tblsubject.SubjectClass LIMIT $offset, $no_of_records_per_page";
$query = $dbh -> prepare($sql);
$query->execute();
...
{               ?>   
                          <tr>
                            ...
                            <td><?php  echo htmlentities($row->SubuID);?></td>
                            <td><?php  echo htmlentities($row->ClassName);?> <?php  echo htmlentities($row->Section);?></td>
                            <td><?php  echo htmlentities($row->SubjectName);?></td>
                            <td>
                             <a href="edit-subject-detail.php?editid=<?php echo htmlentities ($row->sid);?>" class="btn btn-primary btn-sm"><i class="icon-eye"></i></a>
                                                 <a href="manage-subjects.php?delid=<?php echo ($row->sid);?>" onclick="return confirm('Do you really want to Delete ?');" class="btn btn-danger btn-sm"> <i class="icon-trash"></i></a>
                            </td> 
                          </tr><?php $cnt=$cnt+1;}} ?>
                        </tbody>
                      </table>
